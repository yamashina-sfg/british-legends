...
const effectsOf = (flaw: TragicFlaw): TragicFlawEffect[] => [
...
export function skillIdsWithTragicFlaw(character: Character, learnedSkillIds: string[]): string[] {
  ...
  return activeSkillId ? [...new Set([...learnedSkillIds, activeSkillId])] : learnedSkillIds;
}
...
export function flawIsAwakened(runtime: FlawRuntime | undefined, hp: number, maxHp: number): boolean {
  ...
  return effectsOf(runtime.flaw).some((effect) => {
...
export function effectiveStats(stats: Stats, runtime: FlawRuntime | undefined, hp: number, maxHp: number): Stats {
  ...
  const next = { ...stats };
  for (const effect of effectsOf(runtime.flaw)) {
...
export function onDefend(runtime: FlawRuntime | undefined): string | null {
  if (!runtime?.flaw.meter) return null;
  const effect = effectsOf(runtime.flaw).find((item) => item.type === 'meterOnCommand' && item.command === 'defend');
