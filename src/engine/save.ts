...
export function loadSlot(slotId: number): SaveData | null {
  try {
    ...
    return normalizeActiveParty({
      ...
      gold: parsed.gold ?? 36,
      items: parsed.items ?? {},
      acquiredItemIds: [...new Set([
        ...(parsed.acquiredItemIds??[]),
        ...Object.entries(parsed.items??{}).filter(([,count])=>count>0).map(([id])=>id),
        ...(parsed.quickSlots??[]).filter((id):id is string=>Boolean(id)),
      ])],
