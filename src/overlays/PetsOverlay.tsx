...
import { PET_ENHANCE_MAX, PET_EVOLVE_LEVEL, petExpForLevel, petStats } from '@/engine/pets';
...
export function PetsOverlay(){
  ...
  return <Window title="LITERARY FAMILIARS — 使い魔" className="pets-overlay">
    ...
    <div className="pets-layout"><div className="pet-list">{pets.length?pets.map((owned)=>{const pet=getPet(owned.petId);const used=save.petSlots?.includes(owned.uid);return <button key={owned.uid} className={`${selected?.uid===owned.uid?'is-selected':''} ${used?'is-used':''}`} onClick={()=>setSelectedUid(owned.uid)}><i>{'◆'.repeat(pet.rank)}</i><strong>{pet.name}</strong><span>Lv{owned.level} +{owned.enhance}</span></button>}):<p>まだ使い魔を獲得していません。対象の怪異を討伐すると低確率で契約できます。</p>}</div>
      {selected&&definition&&stats&&<section className="pet-detail"><header><div className={`pet-orb is-${definition.element}`}>◉</div><div><small>RANK {definition.rank} / {definition.element.toUpperCase()}</small><h3>{definition.name}</h3><span>Lv {selected.level}/99　鍛錬 +{selected.enhance}/{PET_ENHANCE_MAX}</span></div></header><div className="pet-stats">{Object.entries(stats).map(([key,value])=><span key={key}><small>{key.toUpperCase()}</small><b>{value}</b></span>)}</div><div className="pet-skills">{definition.skillIds.slice(0,2).map((id)=><span key={id}><b>{getSkill(id).name}</b><small>{getSkill(id).description}</small></span>)}</div><p>EXP {selected.exp}/{petExpForLevel(Math.min(99,selected.level+1),definition.expScale)}　召喚中は英雄EXPの30%を追加獲得</p><div className="row"><Button primary onClick={()=>setPetSlot(slot,selected.uid)}>枠{slot+1}に召喚</Button><Button disabled={selected.enhance>=PET_ENHANCE_MAX||(save.inventory[definition.enhanceMaterialId]??0)<selected.enhance+1} onClick={()=>trainPet(selected.uid)}>鍛錬（{getMaterial(definition.enhanceMaterialId).name} {save.inventory[definition.enhanceMaterialId]??0}/{selected.enhance+1}）</Button><Button disabled={!definition.evolveTo||selected.level<PET_EVOLVE_LEVEL||save.gold<500*definition.rank} onClick={()=>evolvePet(selected.uid)}>進化</Button></div></section>}
