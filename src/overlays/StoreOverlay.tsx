...
import { boostRemainingLabel, normalizeCommerce } from '@/engine/commerce';
    ...
    <Window title="BIBLIOTHECA STORE" className="store-overlay">
      ...
      <div className="store-items">
        {tab==='premium'&&<section className="cash-shop">
          ...
          {cashCategory==='boost'&&<div className="cash-shop__boosts"><div className="boost-status"><span>経験値10倍 <b>{boostRemainingLabel(commerce.activeBoosts.expUntil,commerce.activeBoosts.expPausedAt??Date.now())}</b></span><span>ドロップ10倍 <b>{boostRemainingLabel(commerce.activeBoosts.dropUntil,commerce.activeBoosts.dropPausedAt??Date.now())}</b></span></div><Button disabled={(save.items.exp_boost??0)<1} onClick={()=>activateBoost('exp_boost')}>経験値10倍を使用 ×{save.items.exp_boost??0}</Button><Button disabled={(save.items.drop_boost??0)<1} onClick={()=>activateBoost('drop_boost')}>ドロップ10倍を使用 ×{save.items.drop_boost??0}</Button></div>}
        ...
        {tab === 'craft' ? EQUIPMENT_RECIPES.map((recipe) => {
          ...
          <article className={`store-item ${save.gold < item.price ? 'is-unaffordable' : ''}`} key={item.id}>
            ...
            <div className="store-item__buy"><b><i>G</i> {item.price}</b>{item.skillId&&<Button disabled={!quickSlotsUnlocked||!acquiredItemIds.has(item.id)} onClick={() => setQuickSlot(selectedQuickSlot, item.id)}>{!quickSlotsUnlocked?'枠未解放':acquiredItemIds.has(item.id)?'登録':'未入手'}</Button>}<Button disabled={save.gold < item.price||(save.items[item.id]??0)>=99} onClick={() => buyItem(item.id)}>{(save.items[item.id]??0)>=99?'所持上限':'購入'}</Button></div>
          </article>
        )) : equipment.map((item) => (
